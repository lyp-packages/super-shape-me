; Original code by Janek Warchoł, extracted from openlilylib:
...
(if (not (defined? 'debug-control-points-cross-size))
    (define debug-control-points-cross-size 0.7))
(if (not (defined? 'debug-control-points-color))
...
(define (special-control-points )
  (lambda (grob)
    (let* ((grob-name (lambda (x) (assq-ref (ly:grob-property x 'meta) 'name)))
           (name (grob-name grob))
           (stil (cond ((or (eq? name 'Slur) (eq? name 'PhrasingSlur))
                        (ly:slur::print grob))
                       ((eq? name 'LaissezVibrerTie)
                        (laissez-vibrer::print grob))
                       ((or (eq? name 'Tie) (eq? name 'RepeatTie))
                        (ly:tie::print grob))))
